...
LOCAL_BATCH_SCRIPT="run_batch.sh"              # path on your Ubuntu machine
LOCAL_SPARK_SCRIPT="batch_base_v2.py"          # path on your Ubuntu machine
...
# ── 4.5. Install Python dependencies inside the container ────────────────────
echo ""
echo "[4.5] Installation des dépendances Python..."
docker exec "${CONTAINER}" pip install influxdb-client happybase --quiet --break-system-packages
if [ $? -ne 0 ]; then
    echo "[ERREUR] Échec de l'installation des dépendances Python"
    exit 1
fi
echo "[OK] influxdb-client + happybase installés"
