...
def influx_ml_writer(batch_df, batch_id):
    rows = batch_df.select(
        "source_ip",
        "event_time",
        "predicted_label",
    ...
    for row in rows:
        ...
        label = _escape_tag(row["predicted_label"] or "unknown")

        line = (
            f"ml_predictions,"
            f"source_ip={_escape_tag(row['source_ip'])},"
            f"predicted_label={label} "
            f"prediction={float(row['prediction'])},"
            f"benign_probability={float(row['benign_probability'])},"
            f"suspicious_probability={float(row['suspicious_probability'])},"
            f"malicious_probability={float(row['malicious_probability'])},"
            f"bytes_xfer={int(row['bytes_xfer'] or 0)}i "
            f"{ts_ns}"
        )
    ...
    try:
        _write_to_influx(lines)
        print(f"[InfluxDB Batch {batch_id}] ml_predictions → {len(lines)} points")

    except Exception as e:
        print(f"[InfluxDB ERROR] ml_predictions batch {batch_id}: {e}")
# ===================== CASSANDRA WRITER =====================
...
# ===================== DETECTIONS =====================
def brute_force(predictions_df):
    return (
        predictions_df.withWatermark("event_time", "2 minutes")
        .filter(
...
def exfiltration(predictions_df):
    return (
        predictions_df.withWatermark("event_time", "1 minutes")
...
def ip_scores(predictions_df):
    ...
    return (
        predictions_df.withWatermark("event_time", "10 minutes")
        .withColumn("attack_type", attack_type_col)
...
def main():
    ...
    ml_model = RandomForestClassificationModel.load(
   		 "hdfs://hadoop-master:9000/models/cybersecurity_rf_model"
	)

    indexed_df = df.withColumn(
	    "protocol_clean",
	    F.upper(F.coalesce(F.col("protocol"), F.lit("TCP")))
	)

    indexed_df = indexed_df.withColumn(
	    "action_clean",
	    F.lower(F.coalesce(F.col("action"), F.lit("allowed")))
	)
    indexed_df = indexed_df.withColumn(
	    "protocol_index",
	    F.when(F.col("protocol") == "TCP", 0.0)
	     .when(F.col("protocol") == "UDP", 1.0)
	     .when(F.col("protocol") == "ICMP", 2.0)
	     .when(F.col("protocol") == "HTTP", 3.0)
	     .when(F.col("protocol") == "HTTPS", 4.0)
	     .when(F.col("protocol") == "SSH", 5.0)
	     .otherwise(6.0)
	)

    indexed_df = indexed_df.withColumn(
        "action_index",
        F.when(F.col("action") == "allowed", 0.0)
         .when(F.col("action") == "blocked", 1.0)
         .otherwise(0.0)
    )
    indexed_df = indexed_df.withColumn(
    	    "is_large_transfer",
    	    F.when(F.col("bytes_xfer") > 20000, 1.0).otherwise(0.0)
    	)
    ...
    predictions_df = ml_model.transform(featured_df)
    predictions_df = predictions_df.withColumn(
	    "predicted_label",

	    F.when(F.col("prediction") == 0.0, "benign")
	    .when(F.col("prediction") == 1.0, "suspicious")
	    .when(F.col("prediction") == 2.0, "malicious")
	    .otherwise("unknown")
	)
    ...
    pipelines = [
        ("brute_force_alerts", brute_force(predictions_df),  "append", influx_brute_force_writer),
        ("realtime_threats",   signatures(predictions_df),   "append", influx_signatures_writer),
        ("data_exfil_alerts",  exfiltration(predictions_df), "append", influx_exfil_writer),
        ("ip_threat_scores",   ip_scores(predictions_df),    "append", influx_ip_scores_writer),
	(
    "ml_predictions",
    ...
    predictions_df.select(
        ...
        F.col("probability_array")[0].cast("double").alias("benign_probability"),

        F.col("probability_array")[1].cast("double").alias("suspicious_probability"),

        F.col("probability_array")[2].cast("double").alias("malicious_probability"),
